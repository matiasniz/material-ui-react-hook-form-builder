...
export { MakeFormUI } from './make-form-ui'
...
const BuildForm = ({
  ...
  const buildForm = () => {
    if (columns === 1) {
      return map(config, ({ showInput = true, ...input }, key) =>
        showInput ? (
          <Box mt={1} key={key}>
            {buildInput({
              ...input,
  ...
  return (
    ...
    >
      ...
      <Box
        display='flex'
        justifyContent={noBackButton ? button.position : 'space-between'}
        pb={4}
        pt={6}
      >
        {noBackButton ? (
          ...
          <Box mr={3}>
            <Button
              ...
              disableElevation
            >
              <Typography>{button.backLabel}</Typography>
            </Button>
          </Box>
        )}
        <Button
